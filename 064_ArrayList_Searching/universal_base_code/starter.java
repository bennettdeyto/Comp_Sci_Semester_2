...
class starter {
	...
	public static void searchArrInt(ArrayList<Integer> arr, int num){
		for(int i = 0; i < arr.size(); i++){
			if(arr.get(i) == (Integer)num){
				System.out.println("Yes, this number exists at the index: " + i);
			}
		}
		return;
	}
	public static void searchArrString(ArrayList<String> arr, String line){
		for(int i = 0; i < arr.size(); i++){
			if(arr.get(i).equals(line)){
				System.out.println("Yes, this string exists at the index: " + i);
			}
		}
		return;
	}
